...
#define putLine( x , sTempLine ) strcpy(sTempLine, x)
#define getLine_w_rtrim( x, sTempLine, y ) \
    getLine(x, sTempLine, y); \
    rtrim(x)
...
static gb_string getLine(gb_string sLine, char *sTempLine, FILE *FSeqFile) {
    gb_string sReturn;
...
static gb_string joinLines(FILE *FSeqFile, char *sTempLine, unsigned int iSpaceLen) {
    char sLine[LINELEN];
    gb_string sTemp, sJoinedLine;

    sJoinedLine = malloc(sizeof(char) * LINELEN);

    getLine_w_rtrim(sLine,sTempLine, FSeqFile);
    ...
    putLine(sLine,sTempLine);

    return realloc(sJoinedLine, sizeof(char) * (strlen(sJoinedLine) + 1));
}

static int parseLocus(gb_string sLocusStr, gb_data *ptGBData, regex_t *ptRegExLocus) {
    /*    
    01-05      'LOCUS'
    06-12      spaces
    13-28      Locus name
...
static void parseDef(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    char sLine[LINELEN];
...
static void parseKeywords(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    char sLine[LINELEN];
    regmatch_t ptRegMatch[3];
	regex_t ptRegExOneLine;
...
static void parseAccession(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    char sLine[LINELEN];
    regmatch_t ptRegMatch[3];
...
static void parseVersion(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    char sLine[LINELEN];
...
static void parseComment(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    ptGBData->sComment = joinLines(FSeqFile,sTempLine, 12);
}

static void parseSource(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    char sLine[LINELEN];
    regmatch_t ptRegMatch[3];
	regex_t ptRegExOneLine;
	regcomp(&ptRegExOneLine, sOneLine, REG_EXTENDED | REG_ICASE);
    ...
    ptGBData->sLineage = joinLines(FSeqFile,sTempLine, 12);
	regfree(&ptRegExOneLine);
}

#define processRef( x, y,sTempLine ) \
    y = NULL; \
    getLine_w_rtrim(sLine,sTempLine, FSeqFile); \
    putLine(sLine,sTempLine); \
    if (strstr(sLine, x) != NULL) y = joinLines(FSeqFile,sTempLine, 12)

static void parseReference(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    char sLine[LINELEN];
    regmatch_t ptRegMatch[3];
    ...
    processRef("  AUTHORS  ", ptReference->sAuthors,sTempLine);
    processRef("  CONSRTM  ", ptReference->sConsrtm,sTempLine);
    processRef("  TITLE  ",   ptReference->sTitle,sTempLine);
    processRef("  JOURNAL  ", ptReference->sJournal,sTempLine);
    processRef("  MEDLINE  ", ptReference->sMedline,sTempLine);
    processRef("  PUBMED  ",  ptReference->sPubMed,sTempLine);
    processRef("  REMARK  ",  ptReference->sRemark,sTempLine);
...
static gb_string checkOrder(gb_string sLocation) {
    gb_string sPosition;

    skipSpace(sLocation);
	sPosition = sLocation;
    if (*sPosition == 'o' && *(sPosition + 1) == 'r') {
        removeRChar(sLocation, ')');
        return sPosition + 6;
    }
    return sLocation;
...
static gb_string checkRemLoc(gb_string sLocation, char *cLocFlag) {
    gb_string sPosition;
	char * pch;
    skipSpace(sLocation);
	sPosition = sLocation;
	pch=strchr(sLocation,':');

    if (pch) {
		*cLocFlag=REMOTELOCATION;
		return sPosition + (pch-sLocation+1);
    }
	*cLocFlag=LOCALLOCATION;
	return sLocation;
}

static int convertPos2Num(gb_string sPositionsFull, unsigned long *lStart, unsigned long *lEnd, char *cDirection, char *cLocFlag) {
    
    register int i;
    int aiPositions[4] = {-2,};
    int iNum = 0;
	
	gb_string sPositions=checkComplement(sPositionsFull);
	
	if (sPositions == sPositionsFull) *cDirection = NORMAL;
    else *cDirection = REVCOM;
	
	sPositions = checkRemLoc(sPositions,cLocFlag);
...
static void parseLocation(gb_string sLocation, gb_feature *pFeature) {
    ...
    unsigned int iLocationNum = 1,iLocalLocationNum=0;
    ...
    */
    ...
    if (convertPos2Num(sLocation,
        ...
        &(((pFeature->ptLocation)+iLocationNum)->lEnd),
		&(((pFeature->ptLocation)+iLocationNum)->cDirection),
		&(((pFeature->ptLocation)+iLocationNum)->cLocFlag)) == 1)
	{
		if((((pFeature->ptLocation)+iLocationNum)->cLocFlag) == LOCALLOCATION) iLocalLocationNum++;
		iLocationNum++;
	}
    ...
    pFeature->lEnd = ((pFeature->ptLocation)+(iLocalLocationNum - 1))->lEnd;
...
static void parseFeature(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
...
/* Parse sequences */
static void parseSequence(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData) {
    register char c;
    char sLine[LINELEN] = {'\0',};
...
static gb_data *_parseGBFF(FILE *FSeqFile, char *sTempLine, regex_t *ptRegExLocus) {
    int i;
    char sLine[LINELEN] = {'\0',};
    ...
    struct tField {
        ...
        void (*vFunction)(FILE *FSeqFile, char *sTempLine, gb_data *ptGBData);
    } atFields[] = {
        {"DEFINITION", parseDef},
    ...
    /* Parse LOCUS line */ 
    if (parseLocus(sLine, ptGBData, ptRegExLocus) != 0) {
        free(ptGBData);
        return NULL;
    }

    while(getLine(sLine, sTempLine, FSeqFile)) {
        if (*sLine == '/' && *(sLine + 1) == '/') break;
        for(i = 0; *((atFields + i)->sField); i++) {
            if (strstr(sLine, (atFields + i)->sField) == sLine) {
                putLine(sLine,sTempLine);
                ((atFields + i)->vFunction)(FSeqFile, sTempLine, ptGBData);
                break;
            }
        }
    }
...
gb_data **parseGBFF(gb_string spFileName) {
    ...
    do {
        ...
        *(pptGBDatas + iGBFSeqPos) = _parseGBFF(FSeqFile,sTempLine,&ptRegExLocus);
